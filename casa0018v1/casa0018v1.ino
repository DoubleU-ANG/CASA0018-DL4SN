#include <casa0018v4_inferencing.h>
...
/* Includes ---------------------------------------------------------------- */
#include <casa0018v3_inferencing.h>
...
void setup()
{
   ...
   // ei_printf("Edge Impulse Sensor Fusion Inference\r\n");
    ...
    if (!BLE.begin()) {
    ei_printf("* Starting Bluetooth® Low Energy module failed!");
    while (1);
  }
  ...
  ei_printf("Arduino Nano 33 BLE Sense (Central Device)");
  ei_printf(" ");
...
void loop()
{
    ...
    if(result.classification[0].value>result.classification[1].value&&result.classification[0].value>result.classification[2].value){
        ei_printf("%s",result.classification[0]);
        ...
        ei_printf("%s",result.classification[1]);
        digitalWrite(RED,LOW);
        ...
        ei_printf("%s",result.classification[2]);
        digitalWrite(RED,HIGH);
        digitalWrite(BLUE,HIGH);
        gesture=2;
    }
